...
export default function Rooms() {
  ...
  // Mock initial room inventory matching spec (Deluxe, Executive, Suite)
  const [rooms, setRooms] = useState([
    { number: 'D101', type: 'Deluxe Room', price: '₹4,500', capacity: '2 Guests', status: 'Available', amenities: ['Wi-Fi', 'AC', 'TV'] },
    { number: 'D102', type: 'Deluxe Room', price: '₹4,500', capacity: '2 Guests', status: 'Occupied', amenities: ['Wi-Fi', 'AC', 'TV'] },
    { number: 'E201', type: 'Executive Room', price: '₹6,500', capacity: '2 Guests', status: 'Available', amenities: ['Wi-Fi', 'AC', 'Mini Fridge'] },
    { number: 'S301', type: 'Suite', price: '₹9,500', capacity: '4 Guests', status: 'Maintenance', amenities: ['Wi-Fi', 'AC', 'Jacuzzi', 'Balcony'] },
  ]);
  ...
  const handleCreateRoom = (e) => {
    ...
    setNewRoom({ number: '', type: 'Deluxe Room', price: '', capacity: '2 Guests', amenities: '' });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-slate-800">Rooms & Inventory</h1>
          <p className="text-xs text-slate-500 mt-0.5">Manage room details, statuses, and amenities</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {rooms.map((room) => (
          <div key={room.number} className="bg-white rounded-xl border border-slate-200 p-5 shadow-xs flex flex-col justify-between space-y-4">
            <div>
              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                <span className="text-lg font-bold text-slate-900">{room.number}</span>
                <span className={`px-2.5 py-0.5 text-xs font-semibold rounded-full border ${STATUS_BADGES[room.status]}`}>
                  {room.status}
                </span>
              </div>
              <div className="mt-3 space-y-1">
                <p className="text-sm font-semibold text-slate-800">{room.type}</p>
                <p className="text-xs text-slate-500">{room.capacity} • <span className="font-semibold text-slate-700">{room.price}</span> / night</p>
              </div>
            ...
            <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-xs">
              <button className="text-slate-600 hover:text-slate-900 font-medium">Edit Details</button>
              <button onClick={() => handleDeleteRoom(room.number)} className="text-rose-600 hover:text-rose-800 font-medium">
                Delete
              </button>
            </div>
          </div>
